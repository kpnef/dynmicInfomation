...
# Optional requested duration in config (can only shorten).
    ...
    # Retro-fill clamp: control how far back the "mid" sample is allowed to be.
    # Only meaningful for dyn-retro runs. Priority:
    #   CLI --retro-max-gap-ms (if provided) > config retro_max_gap_ms (if present) > default 200ms.
    mode_norm = str(run_mode).strip().lower().replace("_", "-")
    if mode_norm == "dyn-retro":
        ...
        if int(v) <= 0:
            v = 200
        # Pass-through only if simulator supports it (keeps backward compatibility).
        if "retro_max_gap_ms" in sig.parameters:
            sim_kwargs["retro_max_gap_ms"] = int(v)
        elif "retro_max_gap" in sig.parameters:
            sim_kwargs["retro_max_gap"] = int(v)
    ...
    return _jsonable({
        ...
        "retro_max_gap_ms": sim_kwargs.get("retro_max_gap_ms", sim_kwargs.get("retro_max_gap")),
