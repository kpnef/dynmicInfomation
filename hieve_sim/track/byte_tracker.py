...
class STrack(BaseTrack):
    ...
    def activate(self, kalman_filter: KalmanFilterXYAH, frame_id: int):
        ...
        self.tracklet_len = 0
        # NOTE (HIEVE sparse-scheduling):
        # In the upstream ByteTrack codebase, some implementations only mark a brand-new track
        # as "activated" when it appears at the first frame (frame_id==1), treating later
        # appearances as "unconfirmed" until a subsequent match.
        #
        # In our simulator, a channel may be scheduled for the first time at an arbitrary
        # frame (e.g. frame 53) due to limited compute and weighted random scheduling.
        # If we keep the original gating, such channels would output *no tracks at all*
        # unless they are scheduled again soon enough to confirm those tracks.
        # This produces the observed pathological case: pred=0 for the whole channel.
        #
...
class BYTETracker:
    ...
    def update(self,results,img: np.ndarray | None = None,feats: np.ndarray | None = None,n: int = 1, ) -> tuple[np.ndarray, list[list[int]]]:
        """Update the tracker with new detections and return the current list of tracked objects."""
        n = int(n) if n is not None else 1
        if n <= 0:
            n = 1  # 你要求：传 0 也要强制 predict 1 次
        self.frame_id += n  # 丢失计数器：predict 多少次，这里就等价走过多少帧
        id_history: list[list[int]] = []
        box_history: list[np.ndarray] = []
        # Expose per-step box history without changing return signature
        self._box_history = box_history
        pred_box_history: list[np.ndarray] = []
        # Expose pre-update predicted boxes (before association with dets)
        self._pred_box_history = pred_box_history
        activated_stracks = []
        refind_stracks = []
        lost_stracks = []
        removed_stracks = []
        ...
        # Predict forward N steps with KF, and record IDs after each step
        for _ in range(n):
            self.multi_predict(strack_pool)
            # “每次PREDICT的结果”按你定义只输出 ID 列表（可见框/激活 track）
            id_history.append([t.track_id for t in tracked_stracks if t.is_activated])
            pred = np.asarray([t.result for t in tracked_stracks if t.is_activated], dtype=np.float32)
            box_history.append(pred)
            pred_box_history.append(pred.copy())
        ...
        final_tracks = np.asarray([x.result for x in self.tracked_stracks if x.is_activated], dtype=np.float32)
        final_ids = [t.track_id for t in self.tracked_stracks if t.is_activated]
        id_history.append(final_ids)  # 末尾追加最终融合结果的 ID 列表
        if len(box_history) > 0:
            box_history[-1] = final_tracks  # override last predicted step with fused result
        else:
            box_history.append(final_tracks)
        return final_tracks, id_history

    def PREDICT(self, n: int = 1) -> tuple[np.ndarray, list[list[int]]]:
        """Predict forward `n` steps without using detections.

        This is used by the half-rate tracker on callbacks where we intentionally do NOT read DET.
        It advances internal KF state and exposes per-step predicted boxes via `pop_box_history()`
        (and the same list via `pop_pred_box_history()`).

        Returns:
            final_tracks: ndarray (K,8) for the last predicted step.
            id_history: list of ID lists for each step, plus a final copy (len = n+1).
        """
        n = int(n) if n is not None else 1
        if n <= 0:
            n = 1
        ...
        box_history: list[np.ndarray] = []
        self._box_history = box_history
        # For predict-only, pred history is identical to output history.
        self._pred_box_history = box_history

        # Use current tracked + lost pool, but do NOT change track states.
        ...
        for _ in range(n):
            self.multi_predict(strack_pool)
            ids = [t.track_id for t in tracked_stracks if t.is_activated]
            id_history.append(ids)
            box_history.append(np.asarray([t.result for t in tracked_stracks if t.is_activated], dtype=np.float32))

        final_tracks = box_history[-1] if box_history else np.zeros((0, 8), dtype=np.float32)
        final_ids = [t.track_id for t in tracked_stracks if t.is_activated]
        id_history.append(final_ids)
        return final_tracks, id_history

    # Backward-compat alias (typo tolerance)
    PREDIC = PREDICT

    def pop_box_history(self) -> list[np.ndarray]:
        """Return and clear the last call's per-step box history.
    
        The list length equals the `n` passed to `update()`.
        Each element is ndarray (K,8): [x1,y1,x2,y2,track_id,score,cls,idx].
        """
        hist = getattr(self, "_box_history", None)
        if hist is None:
            return []
        self._box_history = []
        return hist

    def pop_pred_box_history(self) -> list[np.ndarray]:
        """Return and clear the last call's *pre-update* per-step predicted box history.

        The list length equals the `n` passed to `update()`/`PREDICT()`.
        Each element is ndarray (K,8): [x1,y1,x2,y2,track_id,score,cls,idx].

        Notes:
          - For `update()`, this contains the raw Kalman predictions *before* association with detections.
          - For `PREDICT()`, this is identical to `pop_box_history()`.
        """
        hist = getattr(self, "_pred_box_history", None)
        if hist is None:
            return []
        self._pred_box_history = []
        return hist
