...
def run_one_config(config_path: Path, *, seed: int = 0, max_frames: int = 0, iou_min_top_pct: float = 0.95) -> Dict[str, Any]:
    ...
    if total_ms > SIM_DURATION_CAP_MS:
        total_ms = SIM_DURATION_CAP_MS
...
def _parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=0,
        help=(
            "Optional cap for frames per channel (useful for quick sanity checks). "
            "0 means no cap (full duration)."
        ),
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    configs = sorted(split_dir.glob("*.json"))
    if len(configs) != 24:
        print(f"[WARN] Expected 24 configs in split/, found {len(configs)}")
